...
# Input paths
DATASET_NAME = "debug_real_data"  # Options: calvin, calvin_d, calvin_abc
dataset_path = f"/data/user/wsong890/user68/project/UniVLA/real_data"
language_dir = f"{dataset_path}/{DATASET_NAME}"
vq_dir = f"{dataset_path}/{DATASET_NAME}_codes_256"
gripper_vq_dir = f"{dataset_path}/{DATASET_NAME}_gripper_codes"

# Output paths
output_path = f"{dataset_path}/meta"
normalizer_path = f"/data/user/wsong890/user68/project/UniVLA/configs/normalizer_debug_real_data_256"
output_pkl_file = osp.join(output_path, f"{DATASET_NAME}_norm_256.pkl")
...
result_file = []
for scene in tqdm(os.listdir(language_dir), desc="Processing scenes"):#遍历每个episode
    ...
    # print("action_files",action_files)
    # print("action_files",len(action_files))

    try:
        # for f in action_files:
        #     print(np.load(f)["actions"].shape)
        actions = [np.load(f)["actions"] for f in action_files]
