...
def main(dataset_path, output_path, normalizer_path, output_filename):
    ...
    for scene in tqdm(os.listdir(language_dir)):
        if not scene.startswith("libero_10"):
    ...
    # print("Q01:", stats.q01)
    # print("Q99:", stats.q99)
    # json.dump(stats, open("/data/user/wsong890/user68/project/UniVLA/configs/normalizer_libero/norm_stats.json", "w"))
    ...
    stats = json.load(open("/data/user/wsong890/user68/project/UniVLA/configs/normalizer_libero/norm_stats.json"))["norm_stats"]["libero"]
    for scene in result_file:
        action = scene["action"]
        # Normalize to [-1, 1] using Q01 and Q99 as bounds
        normalized = 2 * (action - np.array(stats["q01"])) / (np.array(stats["q99"]) - np.array(stats["q01"]) + 1e-8) - 1
        scene["action"] = np.clip(normalized, -1, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_path", type=str, default="/share/user/iperror/data/univla/processed_data", help="Root path to dataset.")
    parser.add_argument("--output_path", type=str, default="/share/user/iperror/data/univla/processed_data/meta", help="Path to save normalized data.")
    parser.add_argument("--normalizer_path", type=str, default="configs/normalizer_libero_10", help="Path to save normalization stats.")
    parser.add_argument("--output_filename", type=str, default="libero_10_norm_all.pkl", help="Filename for normalized pickle output.")
