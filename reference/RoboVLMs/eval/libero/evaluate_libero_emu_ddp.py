...
from model_wrapper_emu import EmuVLAModel, EmuVLAModel_i_ia_dis,EmuVLAModel_i_ia_dis_2stage
...
def evaluate(
    ...
    run_id = f"{task_suite_name}-{world_size}-{local_rank}"
    ...
    for task_id in range(num_tasks_in_suite):
        ...
        log_file.write(f"\nTask: {task_description}\n")
        #构建0到num_trials_per_task的列表
        episode_indices = list(range(num_trials_per_task))
        episode_indices=episode_indices[local_rank::world_size]
        ...
        log_file.flush()
        task_successes_dict[task_description] = float(task_successes) / float(task_episodes)
    log_file.write(f"Task successes dict: {task_successes_dict}\n")
    for k in sorted(task_successes_dict):
        v = task_successes_dict[k]
        # 若是数值就按三位小数打印，否则原样
...
def parser_args():
    ...
    parser.add_argument("--use_norm_single", action="store_true")
    parser.add_argument("--action_tokenizer", type=str, default=None)
    parser.add_argument("--window_size", type=int, default=1)
    parser.add_argument("--action_chunk", type=int, default=10)
    parser.add_argument("--max_action_len", type=int, default=70)
...
def main():
    ...
    if args.dis_i2a_2stage:
        model = EmuVLAModel_i_ia_dis_2stage(
            ...
            use_norm_single=args.use_norm_single,
            fast_path=args.action_tokenizer,
        )
    elif args.dis_i2a:
    ...
    # sr_path = os.path.join(eval_log_dir, f"success_rate_libero.txt")
